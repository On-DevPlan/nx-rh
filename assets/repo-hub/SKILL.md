---
name: repo-hub
description: 通过 nx-rh（npx-repo-hub）统一管理本机 git 仓库与 Agent Skill。当需要登记或扫描仓库、查看 git 状态与差异、拉取/推送、处理冲突文件，或需要识别、同步、物化 skill 时使用本技能。所有能力都同时提供同构 CLI（加 --json 供程序消费）与本地 Web 面板。
agent_created: true
---
